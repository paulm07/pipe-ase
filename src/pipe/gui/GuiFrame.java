...
import edu.fiu.scis.cadse.json.JSONFactory;
import generator.code.behaviour.BehaviourCodeGenerator;
...
import translation.ToJavaProject;
...
public class GuiFrame extends JFrame implements ActionListener, Observer {
  ...
  private FileAction createAction, openAction, closeAction, saveAction,
      ...
      exportTNAction, exportPSAction, importAction, exportJavaProjectAction, exportJavaProjectBehaviorAction;
  ...
  private TypeAction continuousPlaceAction;
  private TypeAction continuousTransitionAction;

  private AnimateAction startAction, stepforwardAction, stepbackwardAction,
      randomLowLevelAction, randomHighLevelAction, continuousHighLevelAction, randomAnimateAction, modelCheckingAction, convertZ3Action;
...
//      exportMenu.setIcon(new ImageIcon(GuiFrame.class.getResource("/Images/Export.png")));
    ...
    addMenuItem(exportMenu, exportJavaProjectAction = new FileAction("JavaProject", "Export the net to a Java Project", ""));
    addMenuItem(exportMenu, exportJavaProjectBehaviorAction = new FileAction("Behavior JavaProject ", "Export the net to a Java Project following Behavior paradigm", ""));
...
//      }
    ...
    addMenuItem(drawMenu, continuousPlaceAction = new TypeAction("continuousplace", Pipe.CONT_PLACE, "Add a continuous place", "shift P", true));
    addMenuItem(drawMenu, transAction = new TypeAction("Immediate transition", Pipe.IMMTRANS, "Add an immediate transition", "I", true));
    addMenuItem(drawMenu, continuousTransitionAction = new TypeAction("continuoustransition", Pipe.CONT_TRANS, "Add a continuous transition", "shift I", true));
...
//      zoomMenu.setIcon(new ImageIcon(GuiFrame.class.getResource("/Images/Zoom.png")));
    ...
    addMenuItem(animateMenu, continuousHighLevelAction = new AnimateAction("ContinuousHighLevel", Pipe.CONTINUOUSHIGHLEVEL, "Randomly fire a high level transition", "6"));
...
//      addButton(toolBar,redoAction);
    ...
    addButton(drawingToolBar, continuousPlaceAction);
    addButton(drawingToolBar, transAction);
    addButton(drawingToolBar, continuousTransitionAction);
...
//      addButton(animationToolBar, randomAnimateAction);
    ...
    addButton(animationToolBar, continuousHighLevelAction);
    addButton(animationToolBar, randomAnimateAction);
  ...
  private void enableActions(boolean status) {
    ...
    continuousPlaceAction.setEnabled(status);
    ...
    continuousTransitionAction.setEnabled(status);
...
//        CreateGui.s
  ...
  public void saveTmpOperation(boolean forceSaveAs) {
...
//	    if (modelFile.getPath().equals(modelFile.getName()))
//	    	filename = runId + "-" + modelFile.getName();
//	    else 
	    String loc = modelFile.getParent();
	    String dir = "/pipe_Tmp";
	    
	    System.out.println("\n" + loc + dir + "\n");
	    
	    File tmpDir = new File(loc + dir);
	    boolean dirExists = tmpDir.mkdir();
	    
	    if (!dirExists && !(new File(loc + dir).exists()) )	{
	    	//return;
	    }
	    
	    filename = tmpDir.getName() + modelFile.getName() + "-" + runId + "-" + "tmp";
	    if (runId == 0)
	    	filename = modelFile.getPath() + "-" +  "ORIGINAL";
	    if (filename != null) 
	    	saveNet(new File(filename));
	    if (runId == 0)
	    	createNewTab(new File(filename), false);
...
//      CreateGui.removeAnimationHistory();
  ...
  public void restoreMode() {
    ...
    continuousPlaceAction.setSelected(mode == Pipe.CONT_PLACE);
    transAction.setSelected(mode == Pipe.IMMTRANS);
    continuousTransitionAction.setSelected(mode == Pipe.CONT_TRANS);
...
//          stepbackwardAction.setEnabled(animBox.isStepBackAllowed());
          break;
        case Pipe.CONTINUOUSHIGHLEVEL:
          CreateGui.getAnimator().animateContinuous();
          System.out.println("$$$$$ $_$ *_* CONT");
          saveTmpOperation(true);
          break;
...
//          animBox.clearStepsForward();
    ...
    private void showModelCheckingOptionsWithSpin() {
      ...
      final JComboBox<String> optionBox = new JComboBox<>(new String[]{"Single process", "Separate processes", "Group as a process", "Agent oriented"});
      final String[] translatioTypes = new String[]{"single", "separate", "grouped", "agent"};
      optionBox.addActionListener(new ActionListener() {
        @Override
        public void actionPerformed(final ActionEvent e) {

          SpinTranslationConfig config = loadTranslationConfig(translatioTypes[optionBox.getSelectedIndex()]);
          if (config == null) {
            java.util.List<Transition> transitions = Arrays.asList(appModel.getTransitions()).stream()
                .filter(t -> !t.isContinuous())
                .collect(Collectors.toList());

            SpinTranslationConfig.Builder builder = SpinTranslationConfig.builder()
                .numberAs("byte")
                .stringAs("mtype");

            if (optionBox.getSelectedIndex() == 1) {
              transitions.forEach(c -> {
                builder.havingGroupOf(Arrays.asList(c.getName()));
              });
            }
            else if (optionBox.getSelectedIndex() == 3) {
              Map<String, java.util.List<String>> agentsMap = new HashMap<>();
              for (Transition transition : transitions) {
                String agent = transition.getContainerNet();
                if (!agentsMap.containsKey(agent)) {
                  ArrayList<String> list = new ArrayList();
                  agentsMap.put(agent, list);
                  builder.havingGroupOf(list);
                }
                agentsMap.get(agent).add(transition.getName());
              }
            }
            else {
              builder.havingGroupOf(transitions.stream().map(Transition::getName).collect(Collectors.toList()));
            }
            builder.isHybrid(appModel.getTransitionsCount() != transitions.size());
            config = builder.build();
          }
          showTranslationWindow(config);
          configDialogue.setVisible(false);
        }
      });
    ...
    private SpinTranslationConfig loadTranslationConfig(final String pType) {
      File modelFile = CreateGui.getFile();
      ...
      File configFile = new File(modelFile.getParent(), String.format("%s_spin_config_%s.json", fileName, pType));
      if (configFile == null || !configFile.exists()) {
        configFile = new File(modelFile.getParent(), String.format("%s_spin_config.json", fileName));
      }

      if (configFile != null && configFile.exists()) {
        try {
          return JSONFactory.getsInstance().readFromFile(configFile, SpinTranslationConfig.class);
        } catch (IOException e) {
          e.printStackTrace();
        }
      }
    ...
    private void showTranslationWindow(final SpinTranslationConfig pConfig) {
      Promela promela = new Promela(new PNPromela(pConfig));
...
//         if (!isSelected()){
      ...
      placeAction.setSelected(this == placeAction);
      continuousPlaceAction.setSelected(this == continuousPlaceAction);
      transAction.setSelected(this == transAction);
      continuousTransitionAction.setSelected( this == continuousTransitionAction);
      timedtransAction.setSelected(this == timedtransAction);
      arcAction.setSelected(this == arcAction);
      inhibarcAction.setSelected(this == inhibarcAction);
      tokenAction.setSelected(this == tokenAction);
      deleteTokenAction.setSelected(this == deleteTokenAction);
      rateAction.setSelected(this == rateAction);
      markingAction.setSelected(this == markingAction);
      selectAction.setSelected(this == selectAction);
      annotationAction.setSelected(this == annotationAction);
      dragAction.setSelected(this == dragAction);
...
//      }
  ...
  class FileAction extends GuiAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      else if (this == exportJavaProjectAction) {
        try {
          new ToJavaProject().translate(CreateGui.appGui.appModel);
          JOptionPane.showConfirmDialog(GuiFrame.this, "Completed exporting as a Java project", "message", JOptionPane.OK_OPTION);
        } catch (IOException pE) {
          JOptionPane.showMessageDialog(GuiFrame.this,
                  "Error while exporting as Java Project. "+pE.getLocalizedMessage(),
                  "Conversion of PTNet to Java project",
                  JOptionPane.ERROR_MESSAGE);
          pE.printStackTrace();
        }
      }

      else if (this == exportJavaProjectBehaviorAction) {
        try {
          new BehaviourCodeGenerator().generate(CreateGui.appGui.appModel);
          JOptionPane.showConfirmDialog(GuiFrame.this, "Completed exporting as a Java project", "message", JOptionPane.OK_OPTION);
        } catch (IOException pE) {
          JOptionPane.showMessageDialog(GuiFrame.this,
                  "Error while exporting as Java Project. "+pE.getLocalizedMessage(),
